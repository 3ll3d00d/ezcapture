...
HRESULT r210_rgb48::WriteTo(VideoFrame* srcFrame, IMediaSample* dstFrame)
{
	...
	r210::convert(sourceData, reinterpret_cast<uint16_t*>(outData), width, height, mPixelsToPad);

	#ifndef NO_QUILL
	auto execTime = swt.elapsed_as<std::chrono::microseconds>().count() / 1000.0;
	LOG_TRACE_L3(mLogData.logger, "[{}] Converted frame to RGB48 in {:.3f} ms", mLogData.prefix, execTime);
	#endif
