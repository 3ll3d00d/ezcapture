	                  ...
	                  int width, int height, bench_mode mode)
	{
		...
		try
		{
			...
			while (!inFile.eof())
			{
				...
				{
				case avx_pack:
					convert_avx_pack(v210Buffer.data(), strides.srcStride, p210Y, p210UV, width, height);
					break;
				case avx_so1:
					convert_avx_so1(v210Buffer.data(), strides.srcStride, p210Y, p210UV, width, height);
					break;
				case scalar:
					convert_scalar(v210Buffer.data(), strides.srcStride, p210Y, p210UV, width, height);
					break;
				}
